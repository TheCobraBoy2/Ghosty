...
local function execute(f)
	local initialTime = Ghosty.Config.Debug and tick() or nil
...
function Server.Execute(execs, async)
	local startTime = Ghosty.Config.Debug and tick() or nil
	if (startTime) then
		print('BEGIN EXECUTE')
	end
	for _, exe in pairs(execs) do
		exe.Bridges = setmetatable({}, {
			__newindex = Ghosty.Core.Bridge.__newindex
		})
		exe.Ghosty = {
			Server = Ghosty.Server,
			import = Ghosty.import,
			Bridge = Ghosty.Bridge 
		}
		...
		Ghosty.Server[exe.Name or error('Attempted to execute a module without a provided name field')] = exe
	end
	if (startTime) then
...
return setmetatable({}, {
	__call = function(t, v, ...)
		Ghosty = v
		Promise = Ghosty.import('Libraries/Utilities/Promise')
		return Server
	end,
})